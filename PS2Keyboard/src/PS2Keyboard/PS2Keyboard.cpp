...
static char get_iso8859_code(void)
{
	...
	while (1) {
        ...
        if (state & CAPSLOCK) {
          state &= ~CAPSLOCK;
          PS2Keyboard::send(0xED);
          PS2Keyboard::send(0x00);
        }
        else { 
          state |= CAPSLOCK;
          PS2Keyboard::send(0xED);
          PS2Keyboard::send(0x04);
        }
      } else if (s == 0x11 && (state & MODIFIER)) {
				state |= ALTGR;
			}
...
void PS2Keyboard::begin(
  ...
  delay(100);
  PS2Keyboard::send(0xFF); // reset keyboard
  delay(100);
  PS2Keyboard::send(0xED); // command
  PS2Keyboard::send(0x00); // all LEDs off
}
